...
> **本仓库只包含 App 源码。**
> 服务端(Go 后端 `go_backend/`)暂未完善，目前仅测试阶段不开源后端。
...
后端地址**在 App 内运行时配置**，不再依赖 `url.txt`（已随三端打包流程移除）：

- 首次启动进入「设置服务器地址」页引导填写，默认预填开发者示例地址 `http://120.55.41.66:8000`
- 之后在「我的 → 设置 → 服务器地址」可随时修改，重启 App 生效
- 取值规则：本地已保存的地址优先，否则用示例地址

三个平台都是同一套逻辑，**改地址无需重新打包**。地址格式如 `http://192.168.1.10:8000` 或 `https://xxxx.devtunnels.ms`，前后空格会自动去掉。
